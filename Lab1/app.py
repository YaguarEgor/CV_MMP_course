...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Segmentation GUI (Tkinter)")
    ...
    def _build_ui(self):
        ...
        self.view_mode = tk.StringVar(value="boxes")
        self.cmb_view = ttk.Combobox(
            top,
            textvariable=self.view_mode,
            state="readonly",
            values=["boxes", "mask", "panel"],
            width=12
        )
    ...
    def on_segment(self):
        ...
        try:
            ...
            if result.get("eggs", 0) > 0:
                self.var_counts.set(f"Найдено яиц: {result.get("eggs", 0)}")
            else:
                self.var_counts.set(f"Найдено желтых и красных томатов соответственно: {result.get("tomato_yellow", 0)} и {result.get("tomato_red", 0)}")
